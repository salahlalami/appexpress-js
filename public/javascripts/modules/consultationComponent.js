    ...
    axios.get(action).then((res) => {
      ...
      if (res.data.success == 1) {
        ...
        prescriptionTitle.innerHTML = `Ordonnance #1 ${
          res.data.data.patient.name + " " + res.data.data.patient.surname
        }`;
        ...
        if (type == "letter") {
          ...
          letterFrom.querySelector("textarea").value = res.data.data.letter;
        } else {
          prescriptionType.value = type;
  ...
  remove: function (element) {
    ...
    function reportDeleteConfirm() {
      axios.get(action).then((res) => {
        console.log(res);
        if (res.data.success == 1) {
          const className = `.report[data-id="${res.data.data._id}"]`;
          console.log(className);
          const selected = document.querySelector(className);
          console.log(selected);
          if (selected) {
...
const consultationComponent = {
  init: function (component) {
    ...
    savePrescription.addEventListener(
      ...
      function (event) {
        ...
        axios.post(form.action, prescriptionData).then((res) => {
          // Rest all prescriptionForm
          form.action = form.dataset.add;
          form.dataset.status = "new";
          [].forEach.call(prescriptionForms, function (prescriptionForm) {
  ...
  info: function (component, res, filter = ["new"]) {
    console.log(res);
